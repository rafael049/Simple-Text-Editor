...
int editorReadKey() {
    ...
    while ((nread = read(STDIN_FILENO, &c, 1)) != 1){
        if (nread == -1 && errno != EAGAIN) // (EAGAIN) sometimes read time out causes an error
            die("read");
    }
...
void editorInsertNewline() {
    if (E.cx == 0) {
        ...
        row = &E.row[E.cy]; // editorInsertRow usa realloc, pode mudar o endereço
        row->size = E.cx;
        row->chars[row->size] = '\0';
        editorUpdateRow(row);
    }
...
char *editorRowsToString(int *buflen) {
    int totlen = 0;
    int j;
    for (j = 0; j < E.numrows; j++)  // TODO converter para multiplicacao?
...
void editorScroll(){
    ...
    if (E.rx >= E.colloff + E.screencols) {
        E.colloff = E.rx - E.screencols + 1;
    }
}
...
void editorRefreshScreen() {
    ...
    char buf[32];
    snprintf(buf, sizeof(buf), "\x1b[%d;%dH", (E.cy - E.rowoff) + 1, (E.rx - E.colloff) + 1);
